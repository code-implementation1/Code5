...
class RPNHead(nn.Cell):
    ...
    def get_targets(
        ...
        train_img_metas = ops.concat(
            (img_metas[::, 2:4], ops.ones_like(img_metas[::, 0:1])),
            axis=1
        )
        for i in range(self.train_batch_size):
            ...
            for j in range(self.num_layers):
                res = self.cast(
                    self.CheckValid(
                        self.anchor_list[j],
                        self.squeeze(train_img_metas[i:i + 1:1, ::])
                    ),
                    ms.int32
                )
    ...
    def loss(
        ...
        for i in range(self.num_layers):
            ...
            loss_reg_item = loss_reg_item * self.rpn_loss_reg_weight
            loss_cls_item = loss_cls_item * self.rpn_loss_cls_weight
