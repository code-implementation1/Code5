...
class Shared2FCBBoxHead(nn.Cell):
    ...
    def loss(
        ...
        """
        pos_labels = self.cast(
            ops.logical_and(ops.gt(labels, 0), weights), ms.int32
        )
        bbox_weights = pos_labels * labels
        labels = self.onehot(
            labels, self.num_classes + 1, self.on_value, self.off_value
        )
        ...
        loss_cls = (
            ops.reduce_sum(loss_cls, (0,)) /
            (ops.reduce_sum(weights, (0,)) + self.eps)
        )
        ...
        loss_reg = (
            ops.reduce_sum(loss_reg) /
            (ops.reduce_sum(bbox_weights) + self.eps)
        )
        loss_cls = self.loss_cls_weight * loss_cls
        loss_reg = self.loss_reg_weight * loss_reg
    ...
    def get_targets(
        ...
        Returns:
            ...
            Sequence[ms.Tensor]: Positive indices for gt data.
        ...
        for i in range(self.train_batch_size):
            ...
            (
                bboxes, deltas, labels, mask, pos_bboxes, pos_ind, pos_labels,
                pos_mask
