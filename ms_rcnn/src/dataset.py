...
def preprocess_coco(
    ...
    # input_data = prepare_segm_masks(input_data, config)
...
def resize_column_train(data, config):
    """Resize image in train mode."""
    ratio_range = config.ratio_range
    scale = (config.img_height, config.img_width)
    scale = random_sample_ratio(scale, ratio_range)

    img, scale_factor = rescale_with_tuple(img=data['image'], scale=scale)

    new_data = data.copy()
    new_data['image'] = img
    new_data['scale_factor'] = np.array(scale_factor)
    new_data['resized_shape'] = img.shape[:2]
...
def resize_annotation(data, config):
    ...
    h = int(h // config.mask_divider)
    w = int(w // config.mask_divider)
    segm_masks = np.array([
...
def pad_annotation(data, config):
    ...
    pad_h = int(pad_h // config.mask_divider)
    pad_w = int(pad_w // config.mask_divider)
