...
class DetectEval(COCOeval):
    ...
    def compute_gt_dt_num(self):
        ...
        for ids in p.catIds:
            gts_cat_id = self.cocoGt.loadAnns(
                self.cocoGt.getAnnIds(catIds=[ids]))
            dts_cat_id = self.cocoDt.loadAnns(
                self.cocoDt.getAnnIds(catIds=[ids]))
    ...
    def evaluate_every_class(self):
        ...
        for i, cat_id in enumerate(cat_ids):
            ...
            label = labels[cat_id]
            class_result = [label, int(round(tp_num)), gt_num,
                            int(round(dt_num)), round(precision, 3),
                            round(recall, 3)]
            result.append(class_result)
        all_precision = sum_tp_num / sum_dt_num
        all_recall = sum_tp_num / sum_gt_num
        all_result = ["all", int(round(sum_tp_num)), sum_gt_num,
                      int(round(sum_dt_num)), round(all_precision, 3),
                      round(all_recall, 3)]
    ...
    def plot_pr_curve(self, eval_result_path):
        ...
        for i, cat_id in enumerate(cat_ids):
            ...
            plt_path = os.path.join(pr_dir,
                                    "pr_curve_" + labels[cat_id] + ".png")
    ...
    def save_images(self, config, eval_result_path, iou_threshold=0.5):
        ...
        for img_id in img_ids:
            ...
            if config.dataset == "coco":
                ...
                im_path_dir = os.path.join(config.voc_root, 'eval',
                                           "JPEGImages")

            assert config.dataset in ("coco", "voc")

            # Return whether the image is precision_ng or recall_ng
            precision_ng, recall_ng = self.evaluate_ok_ng(img_id, cat_ids,
                                                          iou_threshold)
            # Save to ok_images
            if not precision_ng and not recall_ng:
                ...
                im_path_out = os.path.join(im_path_out_dir,
                ...
                if precision_ng:
                    # origin image path
                    im_path = os.path.join(im_path_dir,
                                           img_info[0]['file_name'])
                    assert os.path.exists(im_path), \
                        "{} not exist, please check image directory".format(
                            im_path
                        )
                    # output image path
                    im_path_out_dir = os.path.join(eval_result_path,
                                                   'precision_ng_images')
                    if not os.path.exists(im_path_out_dir):
                        os.makedirs(im_path_out_dir)
                    im_path_out = os.path.join(im_path_out_dir,
                ...
                if recall_ng:
                    ...
                    if not success:
                        raise Exception(
                            'Failed reading image from [{}]'.format(im_path))
                    self._draw_boxes(image, dts, img_id, labels)
                    save_image(im_path_out, image)
        ...
        for k0 in k_list:
            ...
            for a0 in a_list:
                ...
                for maxDet in m_list:
                    ...
                    self.calculate_pr_dict(
                        tp_sum, fp_sum, label, npig, dtScoresSorted,
                        cat_pr_dict, cat_pr_dict_origin, min_score=min_score
                    )
        logging.info("Done")
        return cat_pr_dict, cat_pr_dict_origin

    def calculate_pr_dict(
            self, tp_sum, fp_sum, label, npig, dtScoresSorted, cat_pr_dict,
            cat_pr_dict_origin, min_score=0.1
    ):
        # iou
        for (tp, fp) in zip(tp_sum, fp_sum):
            ...
            f1 = np.divide(2 * (rc * pr), pr + rc,
                           out=np.zeros_like(2 * (rc * pr)),
                           where=pr + rc != 0)
            ...
            if label not in cat_pr_dict.keys():
                cat_pr_dict_origin[label] = [
                    pr[::-1], rc[::-1], f1[::-1], dtScoresSorted[::-1]
                ]
            ...
            for a0 in a_list:
                ...
                for maxDet in m_list:
                    ...
                    if label not in correct_conf_dict.keys():
                        ...
                        logging.info('maxDet: %d areagRng: %d', maxDet,
                                     p.areagRng)
                        break
    ...
    def write_best_confidence_threshold(self, cat_pr_dict, cat_pr_dict_origin,
                                        eval_result_path):
        """
        ...
        result = ["cat_name", "best_f1", "best_precision", "best_recall",
                  "best_score"]
        ...
        for cat_name, cat_info in cat_pr_dict.items():
            ...
            thre_ = [0.003] + [int(i) * 0.001 for i in range(10, 100, 10)] + [
                0.099]
            ...
            if len(max_f1) == 1:
                ...
                elif f1_li.index(max_f1) != len(f1_li) - 1 and f1_li.index(
                        max_f1) != 0:
                ...
                thre_pre = [index for (index, value) in enumerate(f1_li) if
                            abs(value - max(f1_li)) <= 0.001]
    ...
    def plot_mc_curve(self, cat_pr_dict, eval_result_path):
        ...
        for cat_name, cat_info in cat_pr_dict.items():
            ...
            plt.plot(score, precision, linewidth=2, color="deepskyblue",
                     label="precision")

            # 2.recall-confidence
            plt.plot(score, recall, linewidth=2, color="limegreen",
                     label="recall")
            ...
            plt.table(cellText=[score, precision, recall, f1],
                      rowLabels=row_name, loc='center', cellLoc='center',
    ...
    def plot_hist_curve(self, input_data, eval_result_path):
        ...
        for l in correct_conf_dict.keys():
            ...
            if l in incorrect_conf_dict.keys() and len(
                    correct_conf_dict[l]) > 1 and \
                    len(incorrect_conf_dict[l]) > 1:
                gs = gridspec.GridSpec(4, 1)
                plt.subplot(gs[:3, 0])
                correct_conf_dict[l].sort()
                correct_conf_dict[l].reverse()
                col_name_correct = ['number', 'mean', 'max', 'min', 'min99%',
                                    'min99.9%']
                col_val_correct = self.get_col_val(correct_conf_dict, l)
                sns.set_palette('hls')
                sns.distplot(correct_conf_dict[l], bins=50,
                ...
                col_val_incorrect = self.get_col_val(incorrect_conf_dict, l)
                sns.distplot(incorrect_conf_dict[l], bins=50,
                ...
                table = plt.table(cellText=[col_name_correct, col_val_correct,
                                            col_val_incorrect],
                                  rowLabels=row_name,
                ...
                table.scale(1, 1.5)
                plt.savefig(os.path.join(savefig_path, l) + '.jpg')
            elif l in incorrect_conf_dict.keys() and len(
                    incorrect_conf_dict[l]) > 1:
                gs = gridspec.GridSpec(4, 1)
                plt.subplot(gs[:3, 0])
                incorrect_conf_dict[l].sort()
                incorrect_conf_dict[l].reverse()
                col_name_correct = ['number', 'mean', 'max', 'min', 'min99%',
                                    'min99.9%']
                col_val_correct = self.get_col_val(incorrect_conf_dict, l)
                sns.set_palette('hls')
                sns.distplot(incorrect_conf_dict[l], bins=50,
                             kde_kws={'color': 'b', 'lw': 3},
                             hist_kws={'color': 'b', 'alpha': 0.3})
    ...
    @staticmethod
    def get_col_val(data, ind):
        return [
            len(data[ind]), ('%.4f' % np.mean(data[ind])),
            ('%.4f' % max(data[ind])), ('%.2f' % min(data[ind])),
            ('%.2f' % data[ind][int(len(data[ind]) * 0.99) - 1]),
            ('%.2f' % data[ind][int(len(data[ind]) * 0.999) - 1])
        ]
