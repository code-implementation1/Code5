...
from src.blocks.ms_rcnn import MSRCNNInfer
...
def main():
    ...
    for name, meta, tensor in tqdm(data_generator):
        ...
        bboxes_mask[::, [0, 2]] *= meta[1] / meta[3]
        bboxes_mask[::, [1, 3]] *= meta[0] / meta[2]
        ...
        segms_mask = net.net.mask_head.get_seg_masks(
            segms_mask, bboxes_mask, meta[:2]
        )
        bboxes_mask[::, [2, 3]] -= bboxes_mask[::, [0, 1]]
        confs_mask = bboxes_squee[valids_squee, 4]
        labels_mask = labels_squee[valids_squee]
        ious_mask = ious_squee[valids_squee]
        ...
        for box, conf, label, segm, iou in zip(
                bboxes_mask, confs_mask, labels_mask, segms_mask, ious_mask
        ):
