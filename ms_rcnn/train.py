...
from src.blocks.ms_rcnn import MaskScoringRCNN
...
from src.eval_utils import (
    EvalCell, COCOMeanAveragePrecision, PostProcessorMSRCNN
)
...
def main():
    ...
    experiment_name = '_'.join(
        [datetime.now().strftime('%y%m%d'), 'MSRCNN']
    )
    ...
    net = MaskScoringRCNN(config=config)
    logging.info(
    ...
    cb = get_callbacks(
        arch='MSRCNN', logs_dir=ckpt_save_dir / 'logs',
...
def get_metrics(config, model):
    ...
    bbox_post_processor = PostProcessorMSRCNN(
        bbox_normalization=True, segmentation=False
    )
    segm_post_processor = PostProcessorMSRCNN(
